...
// Function to show orders in the cart
            ...
            <div class="flex-1 overflow-y-auto p-4 space-y-4">
                <div class="flex gap-3">
                    ...
                    <div class="flex flex-col text-xs text-gray-800">
                        <p class="font-semibold leading-tight">كشاف ليد 24 وات سقف مشرم 3 نكات سراج</p>
                        <p class="text-[9px] text-gray-500 mt-0.5">النوع: Sirage</p>
                        <p class="text-[10px] text-yellow-600 mt-1 font-semibold">395,00 EGP</p>
                    </div>
                    <button aria-label="حذف المنتج" class="text-gray-500 text-xs self-start" title="حذف المنتج">×</button>
                </div>
            </div>`;
    }
}
...
const posh_order = (id_prodect, price) => {
    ...
    if (!localStorage.getItem('user_id') || localStorage.getItem('user_id') === 'null' || localStorage.getItem('user_id') === '' || localStorage.getItem('user_id') === 'undefined' || localStorage.getItem('user_id') === undefined) {
